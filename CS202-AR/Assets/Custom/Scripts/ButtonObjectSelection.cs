...
public class ButtonObjectSelection : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    {
        foreach (Renderer r in interactableToSpawn.GetComponentsInChildren<Renderer>())
        {
            Debug.Log(_colour);
            // Debug.Log(interactableToSpawn.name + " before: " + ms.color);
            r.material.color = _colour;
            // Debug.Log(interactableToSpawn.name + " after: " + ms.color);
        }
    }
    ...
    private void CopyMaterialsToNewIntsanceWithDefaultColour(Transform _spawnedTransform)
    {
        Renderer[] renderers = _spawnedTransform.gameObject.GetComponentsInChildren<Renderer>();

        foreach (Renderer r in renderers)
        {
            for (int i = 0; i < r.materials.Length; i++)
            {
                r.materials[i] = new Material(previewMaterialSets[i].material);
                r.materials[i].color = previewMaterialSets[i].color;
            }
        }
    }
}
